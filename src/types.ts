...
/**
 * Временные границы именно подписанной исторической линии.
 *
 * Это не утверждение, что после `to` исчезли население, культура или сама
 * территория. Граница означает только, что дальше нельзя продолжать цветную
 * дорожку под прежним названием государства/исторического периода.
 */
export type HistoricalLineSpan = {
  /** Первый год линии включительно; отрицательные значения — до н. э. */
  from: number;
  /** Последний год линии включительно; отрицательные значения — до н. э. */
  to: number;
  /** Хотя бы одна граница является принятой периодизацией, а не точной датой. */
  approximate?: boolean;
  /** Институциональные или академические основания выбранных границ. */
  sources: SourceLink[];
};
...
export type Country = {
  ...
  aliases?: string[];
  /**
   * Период, в котором корректно показывать линию под этим историческим
   * названием. У современных территориальных линий поля нет: они служат
   * сквозной навигацией по истории территории.
   */
  lineSpan?: HistoricalLineSpan;
  /**
   * Исторические контекстные линии, которые показываются здесь, если сама
   * древняя линия не выбрана отдельной колонкой. Италия показывает Древний Рим,
   * Греция — Древнюю Грецию и Македонию. См. data/countries.ts.
   * Это навигационная настройка, а не модель исключительного правопреемства.
   */
  ancestors?: CountryId[];
};
...
export type Track = {
  ...
  countryId?: CountryId;
  layerId?: string;
  /** Границы исторической линии; отсутствуют у слоёв и современных стран. */
  lineSpan?: HistoricalLineSpan;
  /**
   * Дорожка досталась колонке по наследству (Древний Рим в колонке Италии),
   * а не потому, что читатель объединил линии вручную. Различие важно:
   * унаследованную дорожку нельзя «вынести в свою колонку».
   */
  inherited?: boolean;
};
