...
5. У каждой трактовки желательны свои источники.

Готовые примеры: монгольское нашествие, 1492 год, Люблинская уния, атомные бомбардировки,
опиумные войны — [`src/data/viewpoints.ts`](../src/data/viewpoints.ts).

---

## 8. Связи между событиями

Связь — утверждение о **влиянии**, а не о совпадении. Проверочный вопрос:
*можно ли назвать механизм, которым одно повлияло на другое, и подтвердить его источником?*
Если механизм не называется — это просто одновременность, и её показывает сама шкала.

```ts
{
  id: 'rel-gutenberg-luther',
  from: 'de-gutenberg-1455',   // источник влияния
  to: 'de-luther-1517',        // на что повлияло
  kind: 'influence',           // influence | conflict | exchange
...
| --- | --- |
| `influence` | Одно привело к другому: технология, идея, реформа, следствие |
| `conflict` | Стороны одного противостояния или взаимно исключающие итоги |
| `exchange` | Перекличка, сравнение, развилка: похожий вызов — разные ответы |
...
Статьи лежат в [`src/data/articles.ts`](../src/data/articles.ts) под ключом-идентификатором
объекта. Если статьи нет, окно соберётся автоматически из полей карточки — это нормально,
длинные тексты пишутся постепенно.
...
```text
src/data/
  items/<страна>.ts     карточка объекта — обязательно
  articles.ts           развёрнутая статья — по желанию
  viewpoints.ts         трактовки и источники — по необходимости
  relations.ts          связи — по необходимости
  countries.ts          новая страна: id, подпись, пояснение линии, цвет
```

**Добавить объект** — одна запись в файл своей страны. Всё остальное подхватится
автоматически: сортировка, фильтры, кольца дат, детализация по месяцам.

**Добавить страну** — файл в `items/`, строка в `timelineItems.ts`, запись
в `countries.ts`. Больше ничего.
...
8. **Запишите источники** в `sources` — все, по которым проверяли.
